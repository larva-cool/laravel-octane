...
use Laravel\Octane\Workerman\Actions\ConvertWorkermanRequestToIlluminateRequest;
...
class WorkermanClient implements Client, StoppableClient, ServesStaticFiles
{
    ...
    public function canServeRequestAsStaticFile(Request $request, RequestContext $context): bool
    {
        ...
        return $this->fileIsServable($publicPath, $pathToFile,);
    }
    ...
    {
        $response = $this->toPsr7Response($response->response);

        $workermanResponse = new WorkermanHttpResponse();
        ...
        $workermanResponse->withBody($response->getBody());
